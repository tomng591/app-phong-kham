...
export function HistorySessionView({ entry }: HistorySessionViewProps) {
  const { scheduleResult, patients, tasks, doctors, session } = entry;
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <HistoryPatientJourney entry={entry} />
      </div>
      ...
      {scheduleResult.unhandled.length > 0 && (
        <div>
          ...
          <HistoryUnhandledTasks entry={entry} />
        </div>
      )}
    </div>
  );
...
function HistoryPatientJourney({ entry }: HistorySessionViewProps) {
  const { scheduleResult, patients, tasks, doctors, session } = entry;
  ...
  tasksByPatient.forEach((taskList) => {
    taskList.sort((a, b) => a.start_time - b.start_time);
  });
  ...
  const patientsWithTasks = patients.filter((p) => {
    const taskList = tasksByPatient.get(p.id);
...
function HistoryUnhandledTasks({ entry }: HistorySessionViewProps) {
  const { scheduleResult, patients, tasks } = entry;

  const getPatientName = (patientId: string) => {
    const patient = patients.find((p) => p.id === patientId);
    return patient ? `#${patient.daily_id} ${patient.name}` : patientId;
  };
